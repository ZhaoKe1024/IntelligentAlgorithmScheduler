...
class DQNTrainer(object):
    ...
    def __load_checkpoint(self, save_model_path):
        last_epoch = -1
        assert os.path.exists(os.path.join(save_model_path, "optimizer.pth").replace('\\','/')), f"{save_model_path}/optimizer.pth not found!"
        assert os.path.exists(os.path.join(save_model_path, "policynet_bestmodel.pth").replace('\\','/')), "polictnet_bestmodel.pth not found!"
        assert os.path.exists(os.path.join(save_model_path, "targetnet_bestmodel.pth").replace('\\','/')), "targetnet_bestmodel.pth not found!"
        self.policy_net.load_state_dict(torch.load(os.path.join(save_model_path, "policynet_bestmodel.pth").replace('\\','/')))
        self.target_net.load_state_dict(torch.load(os.path.join(save_model_path, "targetnet_bestmodel.pth").replace('\\','/')))
